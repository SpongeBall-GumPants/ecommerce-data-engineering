# etl/generate_lists_and_collections.py
...
def generate_lists_and_collections(n=2000):
    ...
    # 2) COLLECTION (must reference AFFILIATE & SHOP_PRODUCT)
    valid_sp = set(
    ...
    # 3) LISTS (must reference SHOP_PRODUCT & CUSTOMER, name must be unique)
    list_recs = []
    for i in range(n):
        shop_id, product_id = random.choice(list(valid_sp))
    ...
    # 4) SNA (must reference SHOP_PRODUCT & CUSTOMER)
    sna_recs = []
    for i in range(n):
        shop_id, product_id = random.choice(list(valid_sp))
        buyer = random.choice(df_customers["id"].tolist())
